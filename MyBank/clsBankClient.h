...
private:
    ...
    static void _AddClientToArchive(const clsBankClient& Client)
    {
        fstream ArchiveFile;
        ArchiveFile.open("ArchivedClients.txt", ios::out | ios::app);
        if (ArchiveFile.is_open()) {
            ArchiveFile << _ConverClientObjectToLine(Client) << endl;
            ArchiveFile.close();
        }
    }
...
public:
    ...
    static clsBankClient Find(string AccountNumber)
    {
        ...
        if (MyFile.is_open())
        {
            ...
            while (getline(MyFile, Line))
            {
                ...
                if (_Trim(Client.AccountNumber()) == _Trim(AccountNumber))
                {
                    MyFile.close();
                    return Client;
                }
    ...
    bool Delete() {
        ...
        for (auto it = _vClients.begin(); it != _vClients.end(); ++it)
        {
            if (it->AccountNumber() == _AccountNumber) {
                _AddClientToArchive(*it);
                _vClients.erase(it);
                _SaveCleintsDataToFile(_vClients);
    ...
    enSaveResults Deposit(double Amount)
    {
